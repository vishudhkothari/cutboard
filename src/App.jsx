...
/* ─── TRUE ADAPTIVE TDEE (nSuns method) ─────────────────────────
   After 7 days of real data:
   TDEE = avg_daily_cals + (kg_lost_this_week × 7700 / 7)
   More accurate than any formula once real data exists.
──────────────────────────────────────────────────────────────── */
function getAdaptiveTDEE(setup, logs) {
  ...
  const pairedLogs = logs
    .filter(l => l.weight != null && l.meals?.length > 0)
  ...
  if (pairedLogs.length >= 7) {
    ...
    const wtLost  = recent[0].weight - recent[recent.length - 1].weight  // positive = loss
  ...
  if (setup.manualCalTarget) {
    return { target: +setup.manualCalTarget, base, adj: 0, curW, deficit: base - setup.manualCalTarget, isDataDriven, isManual: true }
  }
...
/* ─── CARB CYCLING ───────────────────────────────────────────────
   ...
   Low   (rest day):       TDEE−750,  carbs 18%, fat 82%
   Protein always 2.2g/kg bodyweight regardless of day type.
──────────────────────────────────────────────────────────────── */
...
/* ─── DYNAMIC STEP GOAL ─────────────────────────────────────────
   Base goal (user-set, default 10k) + compensation for
   yesterday's calorie surplus.
   At 74kg: ~0.044 kcal/step  →  capped at +6000 extra steps.
──────────────────────────────────────────────────────────────── */
...
/* ─── COACH INSIGHTS ────────────────────────────────────────────
   Up to 4 priority-ranked insights shown daily.
──────────────────────────────────────────────────────────────── */
function getCoachInsights(setup, logs, todayLog, tdeeData, dayType, macros, stepData) {
  ...
  // 1. Day type (always first)
  insights.push({ icon: '🎯', color: meta.color, msg: meta.desc })

  // 2. Compensation for yesterday
  if (stepData.extra > 0) {
  ...
  // 3. Protein warning (only once enough food is logged)
  ...
  if (wLogs.length >= 7) {
    ...
    if (p7.length >= 4) {
      ...
      if (wkLoss < 0.2)      insights.push({ icon: '📉', color: '#ff8533', msg: `Only ${wkLoss.toFixed(2)}kg lost this week (target ~0.5kg). Tighten calories or add 15 min LISS cardio on rest days.` })
  ...
  if (setup?.carbCycling) {
    if (dayType === 'high')  insights.push({ icon: '💡', color: '#a78bfa', msg: `Schedule your workout today. High-carb days are wasted without training — carbs go straight to glycogen, not fat.` })
    if (dayType === 'low')   insights.push({ icon: '💡', color: '#a78bfa', msg: `Get a 20–30 min fasted walk in the morning. Low-carb + empty stomach = maximum fat oxidation.` })
  }
      ...
      <div style={{ width: '100%', maxWidth: 400, padding: 24 }}>
        ...
        <div style={card()}>
          <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: '#0a0c10', borderRadius: 8, padding: 4 }}>
            {['login','signup'].map(m => (
              <button key={m} onClick={() => { setMode(m); setError('') }} style={{ flex: 1, padding: '8px 0', borderRadius: 6, border: 'none', cursor: 'pointer', fontFamily: F.body, fontSize: 13, fontWeight: 500, transition: 'all 0.15s', background: mode === m ? C.surface : 'transparent', color: mode === m ? C.text : C.textSub }}>
          ...
          <div style={{ display: 'grid', gap: 14 }}>
            ...
            <button style={{ ...btn(true), width: '100%', padding: '12px 0', fontSize: 15, marginTop: 4 }} onClick={submit} disabled={loading}>
...
function Onboarding({ userEmail, onSave, existing, onCancel }) {
  const [f, setF] = useState({
    ...
    age: existing?.age ?? '', height: existing?.height ?? '',
    sex: existing?.sex ?? 'male', activity: existing?.activity ?? 'mod',
    startWeight: existing?.startWeight ?? '', startBF: existing?.startBF ?? '',
    goalBF: existing?.goalBF ?? 12, startDate: existing?.startDate ?? todayStr(),
    stepGoal: existing?.stepGoal ?? 10000,
    carbCycling: existing?.carbCycling ?? false,
        ...
        <div style={{ display: 'grid', gap: 24 }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              {ACTIVITY.map(a => <button key={a.id} style={{ ...btn(f.activity === a.id), textAlign: 'left', width: '100%', padding: '11px 16px' }} onClick={() => set('activity', a.id)}>{a.label}</button>)}
            </div>
          </div>
          ...
          <div style={{ maxWidth: 240 }}>
            <label style={LBL}>Base Daily Step Goal</label>
            <input style={inp()} type="number" step="500" value={f.stepGoal} placeholder="10000" onChange={e => set('stepGoal', +e.target.value)} />
            <div style={{ fontSize: 11, color: C.textSub, marginTop: 6 }}>Extra steps added automatically when you overeat</div>
          </div>
          ...
          <div style={card({ background: '#0c0e14' })}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: f.carbCycling ? 20 : 0 }}>
              <div>
                <div style={{ fontFamily: F.head, fontWeight: 700, fontSize: 15 }}>Carb Cycling</div>
                <div style={{ fontSize: 12, color: C.textSub, marginTop: 4 }}>High carbs on training days, low on rest days. Same weekly average calories.</div>
              </div>
              <button style={btn(f.carbCycling, true)} onClick={() => set('carbCycling', !f.carbCycling)}>{f.carbCycling ? 'ON' : 'OFF'}</button>
            </div>
            {f.carbCycling && (
              <div>
                <label style={LBL}>Training days</label>
                <div style={{ display: 'flex', gap: 8 }}>
                  {DAY_NAMES.map((name, i) => <button key={i} style={{ ...btn(f.trainingDays.includes(i), true), flex: 1, padding: '8px 0', fontSize: 12 }} onClick={() => toggleDay(i)}>{name}</button>)}
                </div>
              </div>
            )}
          </div>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '10px 20px', borderBottom: `1px solid ${C.border}`, background: C.bg, flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: 1, minWidth: 200, maxWidth: 280 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 5, fontSize: 11, color: C.textSub, fontFamily: F.mono }}>
          <span>Day {dayCount} / 60</span><span>{daysLeft}d remaining</span>
        </div>
      </div>
      <div style={{ flex: 1 }} />
      {latestWeight && (
...
/* ═══════════════════════════════════════════════════════════════
   TAB BAR
═══════════════════════════════════════════════════════════════ */
const TABS = [{id:'today',label:'📋 Today'},{id:'nutrition',label:'🥗 Nutrition'},{id:'progress',label:'📈 Progress'},{id:'inbody',label:'🔬 InBody'}]
function TabBar({ tab, setTab }) {
  return (
    <div style={{ display: 'flex', gap: 4, padding: '10px 20px', borderBottom: `1px solid ${C.border}` }}>
      {TABS.map(t => <button key={t.id} style={{ ...btn(tab === t.id, true), borderRadius: 7 }} onClick={() => setTab(t.id)}>{t.label}</button>)}
    </div>
  )
}
...
function TodayTab({ log, adaptiveTDEE, onSave, setup, allLogs }) {
  ...
  const toggleFasting = () => {
    ...
    setLocal(next); onSave(next); if (isFasting) setAddOpen(false)
  }
    ...
    <div style={{ padding: 20, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, maxWidth: 980, margin: '0 auto' }}>
      ...
      <div style={{ ...card({ background: '#0a0c12' }), gridColumn: '1 / -1' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
          ...
          {setup?.carbCycling && (
            <span style={{ background: meta.bg, border: `1px solid ${meta.border}`, color: meta.color, fontSize: 10, fontWeight: 700, padding: '3px 10px', borderRadius: 20, letterSpacing: '0.1em' }}>
              {meta.label}
            </span>
          )}
        </div>
      ...
      <div style={{ ...card(), gridColumn: '1 / -1' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20, marginBottom: 18, textAlign: 'center' }}>
          {local.fasting ? (
            <div style={{ gridColumn:'1/-1', display:'flex', alignItems:'center', justifyContent:'center', gap:16, padding:'10px 0' }}>
              ...
              <div><div style={{ fontFamily:F.head, fontWeight:800, fontSize:22, color:C.blue }}>Fasting Day</div><div style={{ fontSize:13, color:C.textSub, marginTop:2 }}>0 kcal — Full deficit locked in</div></div>
              <div style={{ marginLeft:24, textAlign:'center' }}>
                <div style={{ fontFamily:F.mono, fontSize:28, fontWeight:700, color:C.accent }}>{calTarget}</div>
        ...
        {!local.fasting && (<>
          <div style={{height:6,background:C.border,borderRadius:3,overflow:'hidden'}}><div style={{height:'100%',width:`${pct}%`,background:pct>=100?C.red:C.accent,borderRadius:3,transition:'width 0.4s'}} /></div>
          <div style={{display:'flex',justifyContent:'space-between',marginTop:6,fontSize:11,color:C.textSub}}><span>0</span><span>{calTarget} kcal target</span></div>
        </>)}
      </div>
      ...
      <div style={card()}>
        ...
        <div style={{ marginTop:14, paddingTop:14, borderTop:`1px solid ${C.border}` }}>
          ...
          {stepData.extra > 0 && <div style={{fontSize:11,color:C.orange,background:'#1a0e05',borderRadius:6,padding:'6px 10px',borderLeft:`3px solid ${C.orange}`}}>{stepData.reason}</div>}
          {local.steps != null && <div style={{marginTop:8,height:4,background:C.border,borderRadius:2}}><div style={{height:'100%',width:`${Math.min((local.steps/stepData.goal)*100,100)}%`,background:local.steps>=stepData.goal?C.accent:C.purple,borderRadius:2}} /></div>}
        </div>
        {local.sleep != null && (
          <div style={{ marginTop:14, paddingTop:14, borderTop:`1px solid ${C.border}` }}>
            <div style={{...LBL,marginBottom:8}}>Sleep Quality</div>
            <div style={{display:'flex',gap:8}}>
              {['Poor','OK','Good','Great'].map(q => <button key={q} style={{...btn(local.sleepQuality===q,true),flex:1,padding:'5px 0',fontSize:12}} onClick={() => upd('sleepQuality',q)}>{q}</button>)}
            </div>
          </div>
        )}
      </div>
      ...
      <div style={card()}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}>
          ...
          {adaptiveTDEE.isDataDriven && <span style={{fontSize:10,color:C.accent,background:'#0a1209',border:'1px solid #1a2f12',padding:'3px 8px',borderRadius:20,fontWeight:600,letterSpacing:'0.06em'}}>DATA-DRIVEN</span>}
          {adaptiveTDEE.isManual && <span style={{fontSize:10,color:C.purple,background:'#120e1a',border:'1px solid #2a1a4a',padding:'3px 8px',borderRadius:20,fontWeight:600,letterSpacing:'0.06em'}}>MANUAL</span>}
        </div>
        {setup?.carbCycling && (
        ...
        {[{label:'Maintenance TDEE',val:`${adaptiveTDEE.base} kcal`,color:C.text},{label:'Deficit',val:`−${adaptiveTDEE.deficit} kcal`,color:C.red}].map(({label,val,color}) => (
        ...
        <div style={{display:'flex',justifyContent:'space-between',padding:'13px 0',fontSize:15,fontWeight:600}}>
          ...
          <span style={{fontFamily:F.mono,color:C.accent,fontSize:22}}>{calTarget} kcal</span>
        </div>
      ...
      <div style={{...card(),gridColumn:'1/-1'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            ...
            {local.fasting && <span style={{background:'#0e1e30',border:'1px solid #1a4a7a',color:C.blue,fontSize:11,fontWeight:600,padding:'3px 10px',borderRadius:20}}>FASTING</span>}
          </div>
          <div style={{display:'flex',gap:8}}>
            {!local.fasting && <button style={btn(true,true)} onClick={() => setAddOpen(o=>!o)}>+ Add Meal</button>}
            <button style={{...btn(local.fasting,true),...(local.fasting?{background:'#0e1e30',borderColor:C.blue,color:C.blue}:{})}} onClick={toggleFasting}>{local.fasting ? '↩ Undo Fast' : '🚫 Fasting Day'}</button>
          </div>
        </div>
        ...
        {local.meals.length === 0 ? (
          <div style={{textAlign:'center',padding:'40px 0',color:C.textSub,fontSize:14}}>{local.fasting?'Fasting — no meals today.':'No meals logged yet — add your first!'}</div>
        ) : (
          <>
            ...
            {local.meals.map((m,i) => (
              <div key={i} style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr 1fr 32px',gap:8,padding:'12px 10px',background:'#0c0e16',borderRadius:8,marginBottom:6,alignItems:'center',fontSize:14}}>
                ...
                <button onClick={()=>removeMeal(i)} style={{background:'none',border:'none',color:C.textSub,cursor:'pointer',fontSize:20,padding:0,lineHeight:1}} onMouseEnter={e=>e.currentTarget.style.color=C.red} onMouseLeave={e=>e.currentTarget.style.color=C.textSub}>×</button>
              </div>
            ))}
          ...
          {[{name:'Protein',g:todayP,cals:todayP*4,target:macros.proteinG,color:C.orange},{name:'Carbs',g:todayC,cals:todayC*4,target:macros.carbG,color:C.blue},{name:'Fat',g:todayF,cals:todayF*9,target:macros.fatG,color:C.purple}].map(m => (
            <div key={m.name} style={{marginBottom:18}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:7,fontSize:13}}>
                ...
                <span style={{fontFamily:F.mono,color:m.color}}>{m.g}g <span style={{color:C.textSub,fontSize:11}}>/ {m.target}g</span></span>
              </div>
          ...
          {calHistory.length>1?(
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={calHistory} margin={{top:5,right:5,bottom:5,left:-20}}>
                ...
                <Tooltip {...TT}/>
                <ReferenceLine y={macros.calTarget} stroke={C.red} strokeDasharray="4 4"/>
        ...
        <div style={card()}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:8}}>
            {DAY_NAMES.map((name,i) => {
              ...
              const dt   = getDayType(setup, d.toISOString().slice(0,10))
              const meta = DAY_TYPE_META[dt]
              const m    = getCarbCycleMacros(dt, adaptiveTDEE)
              const isToday = new Date().getDay() === i
              return (
                <div key={i} style={{textAlign:'center',background:isToday?meta.bg:'#0a0c12',border:`1px solid ${isToday?meta.border:C.border}`,borderRadius:8,padding:'10px 6px'}}>
      ...
      {scans.length===0?(
        ...
        <div style={{display:'grid',gap:10}}>
          {[...scans].reverse().map((s,i)=>{
            const prev=scans[scans.length-2-i]; const bfDelta=prev?Math.round((s.bf-prev.bf)*10)/10:null
            return (
...
export default function App() {
  ...
  const Spinner      = s => <div style={{background:C.bg,minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',color:C.accent,fontFamily:F.mono}}>{s}</div>

  if (session === undefined) return Spinner('Loading…')
  if (!session) return <AuthScreen />
  if (!dataReady) return Spinner('Loading your data…')
  ...
  if (!todayLog) return Spinner('Loading today…')
